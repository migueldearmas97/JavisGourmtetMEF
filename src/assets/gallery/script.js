const customInitFunctionGallery = () => {
    console.log('text');
    (function(b) {
        function q(a) { b(a.target).closest(".mbr-slider").hasClass("in") && a.target.playVideo() }

        function m(a) {
            a = b(a.target);
            "undefined" !== typeof b.fn.masonry && a.outerFind(".mbr-gallery").each(function() {
                var a = b(this).find(".mbr-gallery-row").masonry({ itemSelector: ".mbr-gallery-item:not(.mbr-gallery-item__hided)", percentPosition: !0, horizontalOrder: !0 });
                ...
                a.imagesLoaded().progress(function() { a.masonry("layout") })
            })
        }
        ...
        function r() {
            ...
            if (a.length) {
                ...
                a.each(function() {
                    ...
                    a = a >= g ? g : a;
                    g = (c - a * f / g) / 2;
                    k.css({ width: parseInt(a), top: g + 0 })
                })
            }
        }
            ...
            window.onYouTubeIframeAPIReady = function() {
                ...
                b(".video-slide").each(function(a) {
                    ...
                    if (-1 !== b(this).attr("data-video-url").indexOf("vimeo.com")) {
                        var c = { id: b(this).attr("data-video-url"), width: "100%", height: "100%", loop: !0 };
                        ...
                        a = new c("mbr-video-" + a, { height: "100%", width: "100%", videoId: e, events: { onReady: q }, playerVars: { rel: 0 } })
                    }
                    h.push(a)
                })
            }
        ...
        b(document).on("add.cards", function(a) {
            ...
            d.on("click", ".mbr-gallery-filter li > .btn", function(a) {
                ...
                d.find(".mbr-gallery-item").each(function(a, d) {
                    var c =
                        b(this),
                        g = c.attr("data-tags").split(",").map(function(a) { return a.trim() }); - 1 !== b.inArray(f, g) || e.hasClass("mbr-gallery-filter-all") ? c.removeClass("mbr-gallery-item__hided") : (c.addClass("mbr-gallery-item__hided"), setTimeout(function() { c.css("left", "300px") }, 200))
                });
                setTimeout(function() { g.closest(".mbr-gallery-row").trigger("filter") }, 50)
            })
        });
        b(document).on("add.cards changeParameter.cards changeButtonColor.cards", function(a) {
            ...
            d.find(".mbr-gallery-item").each(function(a) {
                (b(this).attr("data-tags") ||
                    "").trim().split(",").map(function(a) { a = a.trim(); - 1 === b.inArray(a, e) && e.push(a) })
            });
            if (0 < d.find(".mbr-gallery-filter").length && b(a.target).find(".mbr-gallery-filter").hasClass("gallery-filter-active") && !b(a.target).find(".mbr-gallery-filter").hasClass("mbr-shop-filter")) {
                ...
                e.map(function(a) { g += '<li><a class="' + f + '" href>' + a + "</a></li>" });
        ...
        b(".mbr-gallery-item").on("click", "a", function(a) { a.stopPropagation() });
        ...
        f.on("hide.bs.modal", function(a) {
            h.map(function(a,
                b) { a.pauseVideo ? a.pauseVideo() : a.pause() })
        })
    })(jQuery);
}
customInitFunctionGallery();